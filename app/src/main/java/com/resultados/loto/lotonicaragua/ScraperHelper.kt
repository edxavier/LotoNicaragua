...
object ScraperHelper {
    ...
    fun apiDiaria(results: List<DiariaResult>): List<LotoResult>{
        ...
        results.forEach { e ->
            ...
            val hora = if (fecha.split("|").size > 1) fecha.split("|")[1] else ""
            entries.add(
                LotoResult(
    ...
    fun apiJuega4(results: List<Juega4Result>): List<LotoResult>{
        val entries: MutableList<LotoResult> = ArrayList()
        results.forEach { e ->
            val fecha = e.dateString
            val hora = if (fecha.split("|").size > 1) fecha.split("|")[1] else ""
            entries.add(
                LotoResult(
                    date = fecha,
                    code = e.drawNumber.toString(),
                    time = hora,
                    result1 = e.winningNumber1,
                    result2 = e.winningNumber2,
                    game = JUEGA4,
                )
            )
        }
